...
"""
    dcc_slider(;kwargs...)
    dcc_slider(children::Any;kwargs...)
    dcc_slider(children_maker::Function;kwargs...)
...
Those elements have the following types:
  ...
  - `style` (Dict; optional)
- `value` (Float64; optional): The value of the input
...
function dcc_slider(; kwargs...)
        available_props = Set(Symbol[:id, :marks, :value, :className, :disabled, :dots, :included, :min, :max, :tooltip, :step, :vertical, :updatemode, :loading_state, :persistence, :persisted_props, :persistence_type])
        wild_props = Set(Symbol[])
        wild_regs = r"^(?<prop>)"

        result = Component("Slider", "dash_core_components", Dict{Symbol, Any}(), available_props, Set(Symbol[]))

        for (prop, value) = pairs(kwargs)
            m = match(wild_regs, string(prop))
            if (length(wild_props) == 0 || isnothing(m)) && !(prop in available_props)
                throw(ArgumentError("Invalid property $(string(prop)) for component " * "dcc_slider"))
            end
...
function dcc_slider(children::Any; kwargs...)
    result = dcc_slider(;kwargs...)
    push!(result.props, :children => Front.to_dash(children))
    return result
end

dcc_slider(children_maker::Function; kwargs...) = dcc_slider(children_maker(); kwargs...)
