...
#' @description Update and track the current window.location object through the window.history state. Use in conjunction with the `dash_core_components.Link` component to make apps with multiple pages.
#' @export
#' @param id  (required)
#' @param pathname pathname in window.location - e.g., "/my/full/pathname" 
#' @param search search in window.location - e.g., "?myargument=1" 
#' @param hash hash in window.location - e.g., "#myhash" 
#' @param href href in window.location - e.g., "/my/full/pathname?myargument=1#myhash" 
#' @param refresh Refresh the page when the location is updated? 

coreLocation <- function(id = NULL, pathname = NULL, search = NULL, hash = NULL, href = NULL, refresh = TRUE) {
...
#' @param children A markdown string (or array of strings) that adhreres to the CommonMark spec 
...
assert_valid_children <- function(..., wildcards = NULL) {
  ...
  if (length(wildcards)) {
    ...
    kids <- kids[!grepl(pattern, names(kids))]
  }
...
append_wildcard_props <- function(component, wildcards = NULL, ...) {
  ...
  pattern <- paste(paste0('^', wildcards), collapse = '|')
  wildcards <- attrs[grepl(pattern, names(attrs))]
  component[['props']] <- c(component[['props']], wildcards)
  component[['propNames']] <- c(component[['propNames']], names(wildcards))
  component
}
